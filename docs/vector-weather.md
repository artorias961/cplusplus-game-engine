...
The particle pool is bounded at 160 sprites plus 24 splashes and three atmospheric entities. Particles now SPAWN at random over time, each small, medium or large, up to a per-preset limit set in `assets/lanebattle/effects.txt`; the pool is only the ceiling. Clear hides it; it remains allocated to avoid allocation churn during switching. Transparent overdraw and per-entity sorting still have a cost, especially with the engine's map-based ECS. Hardware GPU performance has not been benchmarked. The SVG textures consume decoded pixel memory just as PNGs do; smaller source files do not imply less GPU memory.

These are ambient effects: storms have no thunder audio, and presets switch immediately rather than crossfading. (Rain splashes and hail bounces were added later, one per drop that lands — see `independent-animation.md`.) The generated PNG unit sheets are now integrated; see the README and `src/game/lanebattle/Art.h`.
