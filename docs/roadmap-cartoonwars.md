...
| ~~**12**~~ **DONE** | **The sky.** A GRIFFIN that flies; only ARCHER and HERO can reach it. Two flags on a roster row (`flying`, `hitsAir`) and one rule in the queue. | **Nothing** — but it ends the one-dimensional assumption everything since slice 1 rested on. Reach is still measured on x, deliberately: real 2D distance would shrink an archer's 135 to 28 against something overhead. |
